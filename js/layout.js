    ...
    $(document).on("change", "#i", function() {
            ...
            $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var extractionparams = new FormData();
                        console.log('to extract: ', data['data'][0].subtitle.split(' ').join('\n'));
                        extractionparams.append('text', data['data'][0]['subtitle']);
                        if (counter == 1){
                            extractionparams.append('type', 'licence_front');
                        }else if (counter == 2) {
                            extractionparams.append('type', 'licence_back');
                        }else if (counter == 3){
                            extractionparams.append('type', 'eid_front');
                        }else if (counter == 4){
                            extractionparams.append('type', 'eid_back');
                        }else if (counter == 5){
                            extractionparams.append('type', 'registration_front');
                        }else if (counter == 6){
                            extractionparams.append('type', 'registration_back');
                        }else{
                            extractionparams.append('type', 'others');
                        }
                        $.ajax({
                              "url": "https://fcb-mwi.bankbuddy.ai/extract",
                                  "method": "POST",
                                  "async": false,
                                  ...
                                  "success": function (res) {
                                      ...
                                      data['data'][0]['subtitle'] = JSON.stringify(res);
                                      setResponseOnNode(data, responseNode);
                                  }
                        });
                            ...
                            sendText('quote').then(function (response) {
                              ...
                              setResponseOnNode(result, resnode);
                                if(counter == 1){
                                    ...
                                    // jQuery('#attachbtn').css('display', 'block');
                                    var d2 = {'data': [{'title': 'Document Upload', 'subtitle': 'Good going, upload a snapshot of the front side of your emirates ID'}]};
                                    var R2 = createResponseNode();
                                    setResponseOnNode(d2, R2);
                                }else if(counter == 3){
                                    //Vehicle Registration Card
                                    // jQuery('#attachbtn').css('display', 'block');
                                    var d3 = {'data': [{'title': 'Document Upload', 'subtitle': 'Perfect...! Now the back side'}]};
                                    var R3 = createResponseNode();
                                    setResponseOnNode(d3, R3);
                                }else if(counter == 4){
                                    //Vehicle Registration Card
                                    // jQuery('#attachbtn').css('display', 'block');
                                    var d4 = {'data': [{'title': 'Document Upload', 'subtitle': 'Almost there...! Please upload a scanned copy of your Vehicle Registration Card, front side'}]};
                                    var R4 = createResponseNode();
                                    setResponseOnNode(d4, R4);
                                }else if(counter == 5){
                                    //Vehicle Registration Card
                                    // jQuery('#attachbtn').css('display', 'block');
                                    var d5 = {'data': [{'title': 'Document Upload', 'subtitle': 'Last, but not the least, please upload a scanned copy of the back side of your Vehicle Registration Card'}]}
                                    var R5 = createResponseNode();
                                    setResponseOnNode(d5, R5);
                                }else if(counter ==3){
                                    jQuery('#attachbtn').css('display', 'none');
                                    counter = 0;
